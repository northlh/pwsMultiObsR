...
.ig_create_attributes<- function(param_names) {
  ...
  if (length(duplicate_params) > 0) {
    msg <- paste0("The parameters were provided as duplicates : ",
  ...
  if (length(invalid_params) > 0) {
    msg <- paste0("The following parameters are not found in param_attributes: ",
    ...
    if (lines[i] == "####"){
      ...
      if (num_dims == 2){
        param_value_matrix <- as.matrix(as.numeric(
          matrix(data = lines[(i+1):(i+num_values)], #gets all values
...
.ig_perturb_params <- function(sampling_matrix,
  ...
  for (run in seq_len(nrow(sampling_matrix))) { # each perturbation
        ...
        # Handle non-scalar parameters
          ...
          if (is.na(sigma) || sigma == 0) { # case where values are constant
            ...
            new_vals <- matrix(
              ...
              ncol = length(param_vals) / param_data$num_values) # just 1
            ...
            mu_new <- .ig_normalize_reverse(
              x_n = experiment,
              max_x_a = max_val,
              min_x_a = min_val) #uses LHS experiment as new mean
    ...
    run_tagged_path <- file.path(dir_dynamic_input,
                                 paste0("myparam_", run, ".param"))
    .ig_dims_write(dimensions = dimensions,
                  file_path = run_tagged_path,
                  run = run)
    .ig_params_write(parameters = parameters,
                    file_path = run_tagged_path)
...
.ig_sample_morris <- function(r,
  ...
  dimensions <- .ig_dims_read(paste0(dir_dynamic_input,
  ...
  # list_morris["morris_design"] <- morris_design
  # list_morris["morris_samples"] <- sampling_matrix
  # list_morris["morris_params"] <- ms
  # list_morris["param_attributes"] <- param_attributes
  ...
  write.csv(ms, paste0(dir_dynamic_input,"/!morris_params.csv"),
            row.names = FALSE)
  write.csv(param_attributes, paste0(dir_dynamic_input,"/!param_attributes.csv"),
            row.names = FALSE)
  ...
  .ig_perturb_params(sampling_matrix = sampling_matrix,
                     dimensions = dimensions,
...
.ig_sample_mmlhs <- function(nruns,
  ...
  # CREATE LHS OBJECT
  sampling_matrix <- lhs::maximinLHS(n = nruns,
  ...
  msg <- paste("Total time for lhs:: object creation:", time_lhs_total,
               "for", nruns, "samples")
  log_info(msg)
  ...
  # list_mmlhs["mmlhs_params"] <- mmlhs_params
  # list_mmlhs["param_attributes"] <- param_attributes
