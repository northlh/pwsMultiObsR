...
#' end_date <- "2023-09-30"
#'
#' retrieve_q(dir_proj = "/Users/me/mywork/"
...
retrieve_q <- function(
  ...
  if (file.exists(usgs_data_list_filepath)) {
    if (!overwrite) {
      ...
      warning(
        paste("File", usgs_data_list_filepath,
  ...
  for (i in seq_along(usgs_stations$site_ids)) {
    ...
    USGS_data <- dataRetrieval::readNWISdv(
      ...
      endDate = end_date,
      statCd = "00003") #daily mean discharge

    USGS_data <- data.frame(Date = USGS_data$Date,
                            Q_cfs = USGS_data$X_00060_00003)

    USGS_data <- dataRetrieval::addWaterYear(USGS_data)
    USGS_data <- pwsMultiObsR::addWaterDay(df = USGS_data,
                             date_colname = Date,
                             wy_colname = waterYear)
  ...
  jsonlite::write_json(usgs_stations, file.path(obs_path,
...
#'
#' # Input list must follow this format
#' NRCS_stations_test <- list(CO = list(site_ids = c(380, 737),
#                                       hru_ids = c(8,10)),
#'
#'                            CA = list(site_ids = c(356),
#                                       hru_ids = c(8)),
#'
...
#'                            )
#'
#' retrieve_nrcs(dir_proj = "/Users/me/mywork/"
#'               project_name = "East_River",
...
retrieve_nrcs <- function(
  ...
  if (file.exists(nrcs_data_list_filepath)) {
    if (!overwrite) {
      ...
      warning(
        paste("File", nrcs_data_list_filepath,
  ...
  for (state_id in names(nrcs_stations)) {
    ...
    for (i in seq_along(site_ids)) {
      ...
      NRCS_data <- addWaterDay(df = NRCS_data,
                               date_colname = Date,
                               wy_colname = waterYear)
  ...
  jsonlite::write_json(nrcs_stations, file.path(obs_path,
