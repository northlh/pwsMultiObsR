...
#' Helper to read different output list types, wrapped in 'read_output'
...
#' my_proj_names <- c(
#'   "final_BlueRiv", "final_DoloresRiv",
...
#' Blue, Dolores, East, and Taylor river in Colorado, USA. This should be
#' executed immediately after a project (for one of these catchments) is
#' created via fm_create_project()
#'
#' @param dir_root character: parent working directory path
#' @param project_name character: project name
...
load_example_inputs <- function(
  ...
  dir_project <- file.path(dir_root, "projects", project_name)
  ...
  file.copy(
    from = src_smap,
    to = file.path(dir_project, "obs/smapl4"),
    overwrite = FALSE
  )
  ...
  cat("Example files successfully copied to: ", dir_project, " \n")
